...
test("administrator creates a minimal student and sees future service entries", async ({
  ...
  await expect(page.getByRole("heading", { name: "任务监督闭环" })).toBeVisible();
  ...
  const studentName = `自动验收学生${Date.now()}`;
  await page.getByLabel("学生姓名（必填）").fill(studentName);
  ...
  await expect(page).toHaveURL(/\/workspace\/students\/[0-9a-f-]+/, { timeout: 15_000 });
...
test("butler cannot see system management", async ({ page }) => {
  ...
  await page.goto("/workspace/system/users");
  await expect(page.getByText("当前角色没有访问此页面的权限")).toBeVisible();
});
